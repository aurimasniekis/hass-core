"""Test the Automation Light config flow."""
...
async def test_config_flow(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {"name": "My automation_light", "entity_id": input_sensor_entity_id},
    )
    await hass.async_block_till_done()

    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert result["title"] == "My automation_light"
    assert result["data"] == {}
    ...
    assert config_entry.title == "My automation_light"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_automation_light")
